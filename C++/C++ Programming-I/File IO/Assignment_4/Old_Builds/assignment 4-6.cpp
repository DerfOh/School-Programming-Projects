...
bool compareDates(string& currentDate, subscriberRecord& subscriber.experationstring);
bool validDate (subscriber.expirationString);
...
isVailid = validDate(subscriber.expirationString)
    ...
    isExpired = compareDates (currentDate, Subscribes.expirationDate);
...
bool compareDates(string& currentDate, string& subscriber.expiration);
{
    if(strcmp(currentDate, subscriber.expirationstring) == -1)
    {
        return false;
    }
    else
